/**
 * Headless SVG renderer — runs in the extension host (pure TypeScript, no DOM).
 * Returns SVG markup strings that are injected into the webview.
 */
import type { SchematicRoot, ComponentDef, TemplateDef, GraphicPrimitive, ComponentInstance } from '../types';
import { routeAllNets, type Point } from './router';
import { applyTransform, buildSvgTransform, type Rotation } from './transform';

// ── Constants ─────────────────────────────────────────────────────────────────
const MM = 1 / 2.54;
const PX = (mm: number): number => mm * MM;
const FS = 2.5 * MM;
const SW = 0.5 * MM;
const NW = SW * 0.4;
...
function attrStr(a: Attrs): string {
  return Object.entries(a).map(([k, v]) => `${k}="${escAttr(v)}"`).join(' ');
}
function e(tag: string, a: Attrs = {}): string {
  return `<${tag} ${attrStr(a)}/>`;
}
function eWrap(tag: string, a: Attrs, content: string): string {
  return `<${tag} ${attrStr(a)}>${content}</${tag}>`;
}
function txt(tag: string, a: Attrs, text: string): string {
  return eWrap(tag, a, escText(text));
}

// ── Source resolver ───────────────────────────────────────────────────────────
function resolveSource(source: string, schematic: SchematicRoot): string {
  if (!source) return '';
  if (source.startsWith('schematic.titleblock.')) {
    const key = source.replace('schematic.titleblock.', '') as keyof typeof schematic.schematic.titleblock;
    return String(schematic.schematic.titleblock[key] ?? '');
  }
  if (source === 'auto.date')         return new Date().toISOString().slice(0, 10);
...
// ── Title block ───────────────────────────────────────────────────────────────
function renderTitleblock(template: TemplateDef, schematic: SchematicRoot): string {
...
// ── Graphics bbox ─────────────────────────────────────────────────────────────
function graphicsBBox(graphics: GraphicPrimitive[] | undefined): { minX: number; maxX: number; minY: number; maxY: number } {
...
function renderComponent(
  ...
  const infoLines: { text: string; color: string; bold: boolean }[] = [{ text: comp.ref, color: C.ref, bold: true }];
  for (const [key, prop] of Object.entries(def.properties ?? {})) {
    if (!prop || (prop as any).display !== 'shown') continue;
    const override = comp.overrides?.properties?.[key];
    const val = override !== undefined ? override : (prop as any).value;
  ...
  if (needsTransform) {
    return eWrap('g', { transform: buildSvgTransform(cx, cy, rot, fx, fy) }, out);
  }
  return out;
}

// ── Power symbols ─────────────────────────────────────────────────────────────
function renderGnd(x: number, y: number): string {
  const segs: [number, number][] = [[0.4, 0], [0.25, 0.22], [0.1, 0.44]];
...
function renderRail(x: number, y: number, name: string): string {
  let out = e('line', { x1: x, y1: y, x2: x, y2: y - 0.18, stroke: C.wire, 'stroke-width': NW, 'stroke-linecap': 'round' });
...
function renderNetLabel(x: number, y: number, name: string): string {
...
function buildPinMap(
  ...
  for (const comp of schematic.schematic.components ?? []) {
    ...
    for (const pin of def.pins) {
      ...
      if (pin.name !== '~') map.set(`${comp.ref}.${pin.name}`, pt);
    }
  }
  return map;
}
...
function renderNets(
  ...
  const routed = routeAllNets(schematic, componentDefs, pinMap, originPt);
  let out = '';
  for (const net of schematic.schematic.nets ?? []) {
    const pts = (net.pins ?? []).map(id => pinMap.get(id)).filter((p): p is Point => !!p);
    if (net.style === 'gnd') {
      for (const pt of pts) out += renderGnd(pt.x, pt.y);
    } else if (net.style === 'rail') {
      for (const pt of pts) out += renderRail(pt.x, pt.y, net.name);
    } else if (net.style === 'label') {
      for (const pt of pts) out += renderNetLabel(pt.x, pt.y, net.name);
    } else {
...
// ── Page origin ───────────────────────────────────────────────────────────────
export function pageOrigin(template: TemplateDef): Point {
...
export interface RenderResult {
  /** Paper width in grid units */
  paperWidth:  number;
  /** Paper height in grid units */
  paperHeight: number;
  /** SVG markup for the schematic content (components + wires) */
  schematicSvg: string;
  /** SVG markup for the title block */
  titleblockSvg: string;
}

export function renderSchematic(data: {
  schematic: SchematicRoot | null;
  componentDefs: Record<string, ComponentDef>;
 ...
 */
export function buildSvgDocument(result: RenderResult, template: TemplateDef | null): string {
