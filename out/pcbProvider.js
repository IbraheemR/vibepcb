...
exports.PcbEditorProvider = void 0;
const vscode = __importStar(require("vscode"));
const path = __importStar(require("path"));
const pcbRender_1 = require("./pcbRender");
class PcbEditorProvider {
    constructor() {
        this.panels = new Map();
    }
    ...
    buildWebviewHtml(document) {
        ...
        try {
            pcb = JSON.parse(document.getText());
        }
        catch {
            parseError = true;
        }
        ...
        const components = (0, pcbRender_1.resolveComponents)(pcb, pcbDir);
        // Pre-render SVG layers
        const copperSvg = (0, pcbRender_1.renderCopper)(boardH, traces, rects, components);
        const jumpersSvg = (0, pcbRender_1.renderJumpers)(boardH, jumpers);
        const overlaysSvg = (0, pcbRender_1.renderOverlays)(boardH, components);
...
<body>
  ...
  <script>${pcbScript(pageData)}</script>
    ...
    async exportPdfVector(document) {
        let pcb = null;
        try {
            pcb = JSON.parse(document.getText());
        }
        ...
        const uri = await vscode.window.showSaveDialog({
            defaultUri: vscode.Uri.file(path.join(pcbDir, title.replace(/[^a-zA-Z0-9_\-]/g, '_') + '.pdf')),
            filters: { 'PDF': ['pdf'] },
        });
        if (!uri)
            return;
...
exports.PcbEditorProvider = PcbEditorProvider;
// ── Vector PDF builder ────────────────────────────────────────────────────────
...
function buildVectorPdf(boardW, boardH, traces, rects, jumpers, components, title) {
    ...
    const cop = pdfColor(pcbRender_1.COPPER);
    ...
    for (const comp of components) {
        ...
        for (const p of comp.footprint.pads) {
            const [rpx, rpy] = (0, pcbRender_1.rotatePad)(p.x, p.y, comp.rotation);
    ...
    const jw = pdfColor(pcbRender_1.JUMPER_WIRE_COLOR);
    for (const j of jumpers) {
        const padDia = j.pad_diameter_mm ?? pcbRender_1.JUMPER_DEFAULT_PAD_DIA;
    ...
    const ol = pdfColor(pcbRender_1.OUTLINE_COLOR);
    ...
    for (const comp of components) {
        ...
        const rot = (0, pcbRender_1.rotateOutline)(outline.x, outline.y, outline.width, outline.height, comp.rotation);
...
//# sourceMappingURL=pcbProvider.js.map